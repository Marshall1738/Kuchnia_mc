...
import com.example.polskagurom.databinding.FragmentReadyMealBinding
...
class ReadyMealFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.radioGrupaDanieGotowe.setOnCheckedChangeListener { _,  checkedId ->
        ...
        binding.cofajButton.setOnClickListener {
            findNavController().navigate(R.id.action_readyMealFragment_to_menuChoiceFragment)
        }

        binding.doPodsumowania.setOnClickListener {
            findNavController().navigate(R.id.action_readyMealFragment_to_summaryFragment)

            when(selectedOption){
                binding.schabowy.id -> sharedViewModel.send(40)
                binding.golabki.id -> sharedViewModel.send(20)
                binding.rosol.id -> sharedViewModel.send(15)
            }
        }
    }
